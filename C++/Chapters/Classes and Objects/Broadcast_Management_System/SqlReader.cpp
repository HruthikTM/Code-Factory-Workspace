    ...
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK)
    {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            if (sqlite3_column_type(stmt, 3) != SQLITE_NULL)
            {
                ...
                string progTitle = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4));
                string start = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 5));
                string end = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 6));
...
bool SqlReader::createUserTables()
{
    ...
    if (sqlite3_exec(db, adminSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
    {
        cerr << "SQL error creating Admins: " << errMsg << endl;
        sqlite3_free(errMsg);
    }

    if (sqlite3_exec(db, customerSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
    {
        cerr << "SQL error creating Customers: " << errMsg << endl;
        sqlite3_free(errMsg);
    }

    if (sqlite3_exec(db, customerPackageSQL, nullptr, nullptr, &errMsg) != SQLITE_OK)
    {
        cerr << "SQL error creating CustomerPackages: " << errMsg << endl;
        sqlite3_free(errMsg);
    }
...
bool SqlReader::insertUser(const string& type, int id, const string& name, const string& password, const string& phone)
{
    sqlite3* db;
    sqlite3_stmt* stmt;
    string table = (type == "admin") ? "Admins" : "Customers";
    bool success = false;
...
bool SqlReader::verifyUser(const string& type, int id, const string& password)
{
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK)
    {
        ...
        {
            string storedPass = reinterpret_cast<const char*>(sqlite3_column_text(stmt, 0));
...
bool SqlReader::getUser(const string& type, int id)
{
    ...
    if (sqlite3_prepare_v2(db, sql.c_str(), -1, &stmt, nullptr) == SQLITE_OK)
    {
        ...
        if (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            cout << "Name: " << reinterpret_cast<const char*>(sqlite3_column_text(stmt, 1)) << "\n";
            cout << "Phone: " << reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3)) << "\n";
        }
        sqlite3_finalize(stmt);
    }
    ...
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, nullptr) == SQLITE_OK)
    {
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW)
        {
            ...
            cout << "Package ID: " << sqlite3_column_int(stmt, 0)
                 ...
                 << "\nTransaction: " << reinterpret_cast<const char*>(sqlite3_column_text(stmt, 3))
                 << ", Method: " << reinterpret_cast<const char*>(sqlite3_column_text(stmt, 4)) << "\n\n";
        }
        sqlite3_finalize(stmt);
    }
